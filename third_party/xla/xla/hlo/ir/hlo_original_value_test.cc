...
TEST(OriginalValueTest, ElementAccess) {
  ...
  EXPECT_THAT(value.element({0}), Optional(Eq(OriginalArray{"inst1", {1}})));
  EXPECT_THAT(value.element({1}), Optional(Eq(OriginalArray{"inst2", {2}})));

  *value.mutable_element({1}) = OriginalArray{"inst3", {3}};
  EXPECT_THAT(value.element({1}), Optional(Eq(OriginalArray{"inst3", {3}})));
}
